...
bool User::modify_information()
{
	...
	else if (judge == 4)
	{
		...
		for (int i = 0; i < 5; i++)
		{
			if (md5[0] == pointer->data.md5_code[0] && md5[1] == pointer->data.md5_code[1] && md5[2] == pointer->data.md5_code[2] && md5[3] == pointer->data.md5_code[3])
			{
				flag = true;
				break;
			}
			...
			{
				cout << endl
					 << "Please input the right password(You have " << 5 - i << " time left):";
				confidential_input(password);
				md5 = MD5_2(password);
			}
		}
		if (flag == true)
		{
			char password[20] = "\0";
			cout << endl
				 << "Please input your new password(number,letter and captial required, no shorter than 9 characters):" << endl;
			...
			{
				cout << endl
					 << "Please input an appropriate password:" << endl;
				cin.getline(password, 19);
			}
...
bool User::charge()
{
	...
	{
		cout << endl
			 << "Your account:" << this->my_inform.money << endl;
		double add;
